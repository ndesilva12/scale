...
import { ArrowLeft, Flame, Eye, ChevronRight } from 'lucide-react';
import Header from '@/components/layout/Header';
import Card from '@/components/ui/Card';
import { Group } from '@/types';
import { getPopularGroups } from '@/lib/firestore';

export default function PopularGroupsPage() {
  ...
  useEffect(() => {
    const loadGroups = async () => {
      try {
        const popular = await getPopularGroups(50);
        setGroups(popular);
      } catch (error) {
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-3 sm:px-6 lg:px-8 py-3 sm:py-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-lg sm:text-xl font-bold text-white">Popular Groups</h1>
          </div>
        </div>
